...
const StokKeluarGudang = () => {
  ...
  const tambahKeKeranjang = (obat) => {
    ...
    if (obatInCart) {
      setKeranjang(
        keranjang.map((item) =>
          item.id === obat.id ? { ...item, jumlah: item.jumlah + 1 } : item
        )
      );
    } else {
      setKeranjang([...keranjang, { ...obat, jumlah: 1 }]);
    }
  };
  ...
  const updateJumlahKeranjang = (id, jumlah) => {
    setKeranjang(
      keranjang.map((item) =>
        item.id === id ? { ...item, jumlah: Math.max(0, jumlah) } : item
      )
    );
  };
  ...
  const konfirmasiKeluarStok = () => {
    ...
    keranjang.forEach((obat) => {
      ...
      if (obatIndex !== -1) {
        ...
        if (updatedObatData[obatIndex].jumlah >= obat.jumlah) {
          // Kurangi stok yang ada
          updatedObatData[obatIndex].jumlah -= obat.jumlah;
        } else {
          alert(`Stok ${obat.nama} tidak cukup!`);
        }
      }
    });
            ...
            <ul className="space-y-4">
              {keranjang.map((obat) => (
                <li key={obat.id} className="flex justify-between items-center">
                  <span>{obat.nama} ({obat.jenis})</span>
